...
function admin_action_book_locker(int $winner_id, int $post_id) {
	...
	⌛ Du får ett meddelande när du kan hämta i skåpet.', $post_id, $lotten ,$winner_id);
	...
	🔓 Kod till skåpet: <b>'.$locker_code.'</b>', $post_id, $lotten , $author_id);
	loopis_log_level2('		Notifying author: ' . $author_id);
	...
	⏳ Du får ett meddelande när du kan hämta i skåpet.</p>', $post_id, $lotten );
	loopis_log_level2('		Notifying user: Lotten');
...
function admin_action_book_custom(int $winner_id, int $post_id) {
	...
	📍 '.$location.' är den angivna adressen.', $post_id, $lotten , $winner_id);
	loopis_log_level2('		Notifying user: ' . $winner_id . "they're a winner");
	
	// Send notification from LOTTEN to author
	send_admin_notification_email ('❤ '.$winner_name.' har vunnit lottningen! <br>
	📱 '.$winner_name . ' ska nu skicka ett sms till dig för att komma överens om hämtning. <br>
	📍 ' . $location . ' är den angivna adressen @'.$author_name.'.', $post_id, $lotten, $author_id);
	loopis_log_level2('		Notifying author: ' . $author_id);
	
	// Leave comment by LOTTEN
	add_admin_comment ('<p class="lotten">🎲 Dags för lottning! Men vi har bara en deltagare... <br>
	❤ Grattis <span>🔔' . $winner_name . '</span> – du har paxat! <br>
	📱 Du ska nu skicka ett sms till <span>🔔'.$author_name.'</span> för att komma överens om hämtning.</p>', $post_id, $lotten );
...
function admin_action_raffle_locker(array $participants, int $tickets, int $post_id) {
	...
	🙏 Tack för att du loopar! @'.$author_name, $post_id, $lotten, $author_id);
		loopis_log_level2('		Notifying author: ' . $author_id);
	...
	foreach ($participants as $user_id) {
		...
		🍀 '.$tickets.' personer deltog. Bättre lycka nästa gång!', $post_id, $lotten, $loser_id);  
		loopis_log_level2('		Notifying user: ' . $user_id . "they're not a winner");
	}
	// Leave comment by LOTTEN
	...
	👫 Övriga deltagare har fått en lottad plats i kö.</p>', $post_id, $lotten);
...
function admin_action_raffle_custom(array $participants, int $tickets, int $post_id) {
	...
	// Send notification from LOTTEN to winner	
	send_admin_notification_email ('🥳 Grattis @'.$winner_name.' – du har vunnit lottningen! <br>
	📱 Du ska nu skicka ett sms till '.$author_name.' på '.$author_phone. ' för att komma överens om hämtning på '.$location.'.', $post_id, $lotten, $winner_id);
	
	// Send notification from LOTTEN to author
	send_admin_notification_email ('❤ '.$winner_name.' har vunnit lottningen! <br>
	📱 Du ska få ett sms för att komma överens om hämtning på '.$location.'.<br>🙏 Tack för att du loopar! @' . $author_name, $post_id , $lotten, $author_id);
	loopis_log_level2('		Notifying author: ' . $author_id);
